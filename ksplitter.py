    ...
    for letter in karaText:
        if letter in [" ", "!", "?", ",", ";", ":"]:
            if len(karaSplit_array) > 0:
                karaSplit_array[len(karaSplit_array) - 1] = (
                    karaSplit_array[len(karaSplit_array) - 1] + letter
                )
            else:
        ...
        # if letter == "y":
            ...
            if l + nxindx < len(karaText):
                ...
                while ltr != "}":
                    ...
                    karaSplit_array[len(karaSplit_array) - 1] = (
                        karaSplit_array[len(karaSplit_array) - 1] + ltr
                    )
                    nxindx = nxindx + 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="does the k-split for aegisub karaoke (you have to adjust the timing by yourself after splitting)"
    )
    ...
    parser.add_argument(
        ...
        metavar="",
        required=True,
        help="criteria of lines to apply the script on: all , actor or style",
    )
    parser.add_argument(
        "-sv",
        "--selectorvalue",
        metavar="",
        help='if you choose actor or style, specify the name here, use "" for names with spaces "op kara" ',
    )
    ...
    groupSelector.add_argument(
        "-syl",
        "--syllables",
        action="store_const",
        dest="mode",
        const="syl",
        help="makes splitting by romaji syl (default) [only pick one of the three char,syl,word]",
    )
    groupSelector.add_argument(
        "-char",
        "--characters",
        ...
        dest="mode",
        const="char",
        help="makes splitting after each character [only pick one of the three char,syl,word]",
    )
    groupSelector.add_argument(
        "-word",
        "--word",
        ...
        dest="mode",
        const="word",
        help="makes splitting after each word [only pick one of the three char,syl,word]",
    )
    ...
    parser.add_argument(
        ...
        metavar="",
        required=True,
        help="rel or abs path for the .ass file",
    )
    ...
    if not (selector in ["actor", "style", "all"]):
        print(
            'your selector (-s / --selector) has to be either "actor", "style", or "all", you entered: '
            + selector
        )
        print("\n")
        print("exiting...")
    ...
    if selector in ["actor", "style"]:
        if selector_value == None:
            print(
                'your selector is not "all" for "actor", "style",you have to enter the name with -sv/ --selectorvalue = KaraName'
            )
            print("\n")
            print("exiting...")
            sys.exit(-1)
    if not (mode in ["syl", "char", "word"]):
        print(
            'your splitting mode has to be either "syl", "char", or "word", you entered: '
            + mode
        )
        print("\n")
        print("exiting...")
        ...
        for line in sub_file:
            ...
            if input.find("Dialogue") != -1:
                ...
                if match.lower() == selector_value.lower():
                    duration_ds = aegiTimeTOds(inputarray[2]) - aegiTimeTOds(
                        inputarray[1]
                    )
