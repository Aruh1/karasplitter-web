...
/**
 * Extract ASS headers (Script Info, Styles, Events section header) from content
 */
export function extractAssHeaders(content: string): string {
	const lines = content.split(/\r?\n/);
	...
	for (const line of lines) {
		...
		// Stop when we hit the first Dialogue/Comment line
...
/**
 * Generate default Aegisub ASS header template
 */
export function getDefaultAssHeader(): string {
	return `[Script Info]
; Script generated by Karasplitter Web
; https://github.com/Yurasubs/karasplitter-web
Title: Karasplitter Output
ScriptType: v4.00+
WrapStyle: 0
...
/**
 * Generate complete ASS file content with headers and processed lines
 */
export function generateAssFile(
	originalContent: string,
	processedLines: string,
): string {
	// Try to extract headers from original content
	const headers = extractAssHeaders(originalContent);

	// If no headers found, use default template
	if (!headers.trim()) {
		return `${getDefaultAssHeader()}\n${processedLines}`;
	}

	// Combine headers with processed lines
	return `${headers}\n${processedLines}`;
}
...
export function processAssFile(
	...
	// Generate complete ASS file with headers
	const completeContent = generateAssFile(content, outputLines.join("\n"));

	return { content: completeContent, error: null };
}
